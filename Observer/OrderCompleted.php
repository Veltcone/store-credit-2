...
class OrderCompleted implements ObserverInterface
{
    ...
    * @return boolean
    * verifies is particular order is already refunded or not before adding bonous to store credited
    */
    public function isRefunded($orderId){
        $refundCollection = $this->storeRefund->create()
                        ->getCollection()
                        ->addFieldToFilter("order_id",["eq"=>$orderId]);
        return ($refundCollection->count());
    }
    /**
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        try {
            if ($isEnable) {
                ...
                $isRrefundedStatus = $this->isRefunded($observer->getEvent()->getOrder()->getId());
                if ($orderStatus == 'complete' && $isRrefundedStatus == 0) {
